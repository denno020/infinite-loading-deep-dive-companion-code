...
export const useInfiniteLoading = (props) => {
  ...
  const { preload = 'off', partialInfiniteLimit = -1 } = options;
  ...
  const [loadingType, setLoadingType] = useState(() => {
    const preferredLoadingType = localStorage.getItem('user-pref-loading');
    return preferredLoadingType || options.loadingType;
  });
  ...
  const toggleAutomaticLoading = (e) => {
    localStorage.setItem('user-pref-loading', e.target.checked ? 'infinite' : 'manual')
    setLoadingType(prevValue => prevValue === 'manual' ? 'infinite' : 'manual')
  };
